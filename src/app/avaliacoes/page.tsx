...
import { ArrowLeft, Star, MapPin, Sparkles, Send, Edit3, Check, Copy, Loader2 } from 'lucide-react';
...
export default function AvaliacoesPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            // Fallback de contingência caso a cota do dia zere
            setReviews([
  ...
  const handleEdit = (id: number | string, currentText: string) => {
    setEditingId(id);
    setEditedText(currentText);
  };

  const handleSaveText = (id: number | string) => {
  ...
  const handleResponder = async (id: number | string) => {
    try {
      ...
      console.log('Publicado localmente:', e);
    }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-1">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Volume do Mês Atual</span>
            <div className="text-3xl font-black text-slate-900">142 avaliações</div>
            <span className="text-xs text-emerald-600 font-bold flex items-center gap-1">
              ● Sincronização direta com Google Business Profile
            </span>
          </div>
        ...
        {loading ? (
          <div className="bg-white p-12 rounded-3xl border border-slate-200/80 text-center flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-sm font-bold text-slate-600">Buscando últimas avaliações do Google Maps...</p>
          </div>
        ) : (
