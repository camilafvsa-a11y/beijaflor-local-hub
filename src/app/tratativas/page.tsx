        ...
        <div className="bg-gradient-to-r from-[#0f4c81] via-slate-900 to-slate-900 text-white p-8 rounded-3xl shadow-sm relative overflow-hidden">
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl font-black tracking-tight mt-3">Tratativas de Ocorrências</h1>
            ...
            <div className="flex gap-4">
              <div className="bg-white/10 backdrop-blur-sm px-6 py-4 rounded-2xl border border-white/10 text-center">
                <span className="text-xs uppercase font-bold text-blue-200 block">Abertas</span>
                <span className="text-2xl font-extrabold text-amber-300">{items.filter(i => i.status !== 'Concluída').length}</span>
              </div>
              <div className="bg-white/10 backdrop-blur-sm px-6 py-4 rounded-2xl border border-white/10 text-center">
                <span className="text-xs uppercase font-bold text-blue-200 block">Concluídas</span>
                <span className="text-2xl font-extrabold text-emerald-400">{items.filter(i => i.status === 'Concluída').length}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div key={item.id} className="bg-white rounded-3xl border border-slate-200/80 p-6 shadow-sm hover:shadow-md transition-all space-y-4">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-slate-100 pb-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-mono text-xs font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-md">{item.id}</span>
                    <span className="text-xs font-bold px-3 py-1 bg-blue-50 text-[#0f4c81] rounded-md border border-blue-100">{item.canal}</span>
                    <span className="text-sm font-bold text-slate-800">{item.unidade}</span>
                  </div>
                ...
                {editingId === item.id ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-end gap-2">
                      ...
                      <button onClick={() => handleSaveText(item.id)} className="text-xs px-4 py-2 bg-[#0f4c81] text-white font-semibold rounded-xl shadow-sm">Salvar Alteração</button>
                    </div>
                  </div>
                ) : (
                  <p className="text-sm text-slate-700 leading-relaxed font-medium italic">"{item.respostaIA}"</p>
                )}
              </div>
              ...
              {item.status !== 'Concluída' && (
                <div className="flex items-center justify-end pt-1">
                  ...
                  >
                    <Send className="w-4 h-4" /> Publicar Resposta e Finalizar
                  </button>
                </div>
              )}
            </div>
